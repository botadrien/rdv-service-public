...
- Ajoute `bin/fast_specs` pour lancer la suite de tests le plus vite possible en local (`parallel_tests` sur tous les cœurs, historique de durée de la dernière CI verte sur `production` récupéré via `gh` une seule fois au premier lancement si disponible puis auto-entretenu par les runs complets suivants, sinon repli automatique sur un équilibrage par taille de fichier).
